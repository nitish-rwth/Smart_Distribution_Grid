...
def create_mv_network(net_cigre_mv):
    ...
    # Ext-Grid
    pp.create_ext_grid(net_cigre_mv, bus0, vm_pu=1.03, va_degree=0.0, s_sc_max_mva=5000, s_sc_min_mva=5000, rx_max=0.1,
                       rx_min=0.1)

    # Lines
    mv_lines = pd.read_csv('Data/mv_lines.csv', sep=';', header=0, decimal=',')
    ...
    # Switches
    mv_switches = pd.read_csv('Data/mv_switches.csv', sep=';', header=0, decimal=',')
    ...
    # Trafo switches
    pp.create_switch(net_cigre_mv, bus0, trafo0, et='t', closed=True, type='CB', name='Trafo_SW1')
    pp.create_switch(net_cigre_mv, bus0, trafo1, et='t', closed=True, type='CB', name='Trafo_SW2')

    # Residential and Commercial / Industrial Loads
    mv_loads = pd.read_csv('Data/mv_loads.csv', sep=';', header=0, decimal=',')
    ...
    mv_sgens = pd.read_csv('Data/mv_sgens.csv', sep=';', header=0, decimal=',')
    ...
    # print(net_cigre_mv.res_bus)
    # print(net_cigre_mv.res_line)

    # Plotting the network based on power flow results - Not to be used when connected to MV grid
    # simple_plotly(net_cigre_mv)
    to_html(net_cigre_mv, 'Data/MV_net.html')

    # Convert this grid to a json file
    pp.to_json(net_cigre_mv, "Data/MV_grid.json")
